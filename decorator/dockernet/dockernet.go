...
// dockerNetworksOfEngine returns a dockerNetworks object with the networks managed
...
// talk to it, please.
func dockerNetworksOfEngine(ctx context.Context, engine *model.ContainerEngine, allnetns network.NetworkNamespaces) (
	docknets dockerNetworks,
) {
	moby, err := client.New(client.WithHost(engine.API))
	if err != nil {
		slog.Warn("cannot discover Docker-managed networks",
			slog.String("api", engine.API), xslog.Error(err))
		return
	}
	networks, _ := moby.NetworkList(ctx, client.NetworkListOptions{})
	_ = moby.Close()
	...
	slog.Info("found Docker custom networks",
		slog.Int("count", len(networks.Items)),
		slog.Uint64("netns", docknets.engineNetns.ID().Ino),
		slog.String("name", docknets.engineNetns.DisplayName()))
	return
}
...
func Decorate(
	...
	slog.Debug("discovering Docker-managed networks")
	...
	for _, engine := range engines {
		...
		dockerNets[engine.PID] = dockerNetworksOfEngine(ctx, engine, allnetns)
	}
	...
	for _, docknet := range dockerNets {
		...
		for _, netw := range docknet.networks {
			...
			slog.Debug("found passthrough network",
				slog.String("network", netw.Name), slog.String("interface", nifname))
			// Try to locate the Linux network interface related to this Docker
			...
			if !ok {
				...
				// passthrough network.
				dockerclient, err := client.New(client.WithHost(docknet.engine.API))
				if err != nil {
					continue
				}
				netwDetails, err := dockerclient.NetworkInspect(ctx, netw.ID, client.NetworkInspectOptions{})
				_ = dockerclient.Close()
				if err != nil {
					slog.Error("cannot inspect Docker network",
				...
				// can find.
				for cntrID, cntr := range netwDetails.Network.Containers {
					slog.Debug("network interface currently passed into container",
						slog.String("interface", nifname),
						slog.String("container", cntrID))
					// Just get the first and only connected sandbox/container.
					throughpassers[cntrID] = PassedThrough{
						Alias:     netw.Name,
						HwAddress: cntr.MacAddress.String(),
					}
					break
				}
				continue
			}
			slog.Debug("network interface is not yet passed through",
				slog.String("interface", nifname))
			nif := netif.Nif()
			nif.Alias = netw.Name
			// We additionally also label the network interface with the Docker
		...
		if len(throughpassers) > 0 {
			slog.Debug("scanning containers for passed-through network interfaces")
			for _, netns := range allnetns {
				for _, tenant := range netns.Tenants {
					...
					slog.Debug("found passed-through network interface inside container",
						slog.String("interface", passedthrough.Alias),
						slog.String("hw-addr", passedthrough.HwAddress),
						slog.String("container", cntr.Name))
