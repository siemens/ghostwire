...
const portrowkey = (port: ForwardedPort, user: PortUser) => (
    ...
    + `-${port.forwardedPort.toString().padStart(5, '0')}-${port.forwardedAddress.address}`
    + `-${user.pid}`
)
...
const getInitialTableRows = (netns: NetworkNamespace, families: AddressFamilySet) => {
    ...
    return sortTableRows(
        ...
        ['port', 'address', 'forwardedaddress', 'proto', 'forwardedport'])
}
                ...
                <TableBody>
                    {tableRows.map(row =>
                        <TableRow key={row.key}>
                            ...
                            <AddressCell>{(row.port.forwardedAddress.address) || ''}</AddressCell>
                            <PortCell>:{row.port.forwardedPort}</PortCell>
...
export interface ForwardPortTableProps {
...
/**
 * Component `ForwardPortTable` renders a table with details about the
 * forwarded ports used in a specific network namespace.
 */
export const ForwardPortTable = ({ netns, families, hideEmpty }: ForwardPortTableProps) => {
