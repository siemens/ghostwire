...
// We need an outer box (div) with its overflow hidden to avoid the scale
// transformation causing scroll bars to appear during the animation.
// Unfortunately, trying to hide the scrollbars on the ghostbox doesn't
// work.
...
const spectreKeyframes = keyframes({
    '0%, 100%': {
        opacity: 0.6,
        filter: 'blur(1px)',
    },
...
const GhostBox = styled(Box)(({ theme }) => ({
    ...
    '&>svg': {
        width: '12ex',
        maxWidth: '24ex', // ...avoids scrollbars appearing when blurring
        height: '12ex',
        maxHeight: '24ex',
    },
}))
...
export interface GhostProps {
    /** optional margin */
    m?: number
    /** children to render while not refreshing. */
    children: React.ReactNode
}

/**
 * The `Ghost` component normally renders its children, unless there is a
 * discovery refresh ongoing: then an animated Ghostwire mascot is shown while
 * the refresh is going on.
 *
 * > **Note:** the Ghostwire mascot is embedded in the rendered component, so
 * > its can operate fully standalone. This also includes the necessary SVG
 * > linear gradient definitions, for which we automatically create unique ids.
 */
export const Ghost = ({ m, children }: GhostProps) => {
    const [refreshing] = useAtom(discoveryRefreshingAtom)
    const [id] = useState(uniqueId('gw-mascot-'))
