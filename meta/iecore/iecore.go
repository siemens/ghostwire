...
func gatherMetadata(r gostwire.DiscoveryResult) meta.Data {
...
func readEdgeCoreContainerOsrelease(cc *model.Container) map[string]string {
	if cc.Process == nil || cc.Process.Namespaces[model.MountNS] == nil {
		slog.Error("no mount namespace information available for edge-iot-core")
		return nil
	}
	corefs, err := mountineer.New(cc.Process.Namespaces[model.MountNS].Ref(), nil)
	if err != nil {
		slog.Error("cannot access mount namespace of edge-iot-core", xslog.Error(err))
		return nil
	}
	defer corefs.Close()
	...
	if err != nil {
		slog.Error("cannot resolve /etc/os-release-container path in edge-iot-core: %s", xslog.Error(err))
	}
